...
const AllEmails = () => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md mt-6">
      <h2 className="text-2xl font-semibold mb-4 text-gray-700">📩 All Emails</h2>
        ...
        <div className="space-y-3">
          {emails.map((email) => (
            ...
            >
              <div>
                <Link
                  to={`/dashboard/view-email/${email._id}`}
                  className="text-lg font-semibold text-blue-600 hover:underline"
                >
                  {email.subject || "No Subject"}
                </Link>
                <p className="text-sm text-gray-500">{email.email} • {moment(email.createdAt).fromNow()}</p>
              </div>
              <button
